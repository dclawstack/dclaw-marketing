...
@pytest.mark.asyncio
async def test_create_lead(client, test_org_id):
...
@pytest.mark.asyncio
async def test_create_duplicate_lead_within_org_409(client, test_org_id):
...
@pytest.mark.asyncio
async def test_list_leads_scoped(client, test_org_id):
    await client.post(
        f"/api/v1/leads/?organization_id={test_org_id}",
        json={"email": "l1@example.com", "status": "new"},
    )
    response = await client.get(f"/api/v1/leads/?organization_id={test_org_id}")
    assert response.status_code == 200
    assert response.json()["total"] >= 1


@pytest.mark.asyncio
async def test_update_lead(client, test_org_id):
...
@pytest.mark.asyncio
async def test_delete_lead(client, test_org_id):
    create = await client.post(
        f"/api/v1/leads/?organization_id={test_org_id}",
